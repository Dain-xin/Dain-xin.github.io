...
#### **Optional** **接口**(防止null异常的辅助接口)

Optional 不是函数式接口，这是个用来防止NullPointerException异常的辅助类型，这是下一届中将要用到的重要概念，现在先简单的看看这个接口能干什么：
...
// "aaa2", "aaa1"（对s的集合转换为流过滤开头为a的集合遍历输出）
```
...
排序是一个中间操作，返回的是排序好后的Stream。如果你不指定一个自定义的Comparator（自然排序）则会使用默认排序。
...
// 转为流式，map为中间操作，转为大写，排序重写了Comparator接口，然后遍历输出。
```
...
// 并行排序耗时: 472 ms 上面两个代码几乎是一样的，但是并行版的快了50%之多，
...
> Java8提供了很多比较好的接口，还有lambda表达式的使用，遍历对接口实现的写法，不仅可以通过流式编程思想，转换为流stream进行链式编程，实现常见筛选、删除、遍历、拼接方法。
>
> 另一方面我们的
...
新的时间API  **（LocalDate/LocalTime 和 LocalDateTime 类）**

[LocalDateTime参考](https://www.liaoxuefeng.com/wiki/1252599548343744/1303871087444002)
...
> 日历系统中没有时区的日期时间，例如 2007-12-03T10:15:30。 
>
...
>
>  ISO-8601 日历系统是当今世界大部分地区使用的现代民用日历系统。它相当于预测的公历系统，在该系统中，今天的闰年规则适用于所有时间。
>
> 对于当今编写的大多数应用程序，ISO-8601 规则完全适用。
>
> 但是，任何使用历史日期并要求它们准确的应用程序都会发现 ISO-8601 方法不适合。这是一个基于值的类；在 LocalDateTime 的实例上使用身份敏感操作（包括引用相等 (==)、身份哈希码或同步）可能会产生不可预测的结果，应该避免。应该使用equals方法进行比较。实现要求：这个类是不可变的和线程安全的。

3、LocalDateTime提供了一些能访问具体字段的方法。

![image-20220613182243680](https://blog-images-djx.oss-cn-hangzhou.aliyuncs.com/img/202206131822042.png)
...
![image-20220613222337685](https://blog-images-djx.oss-cn-hangzhou.aliyuncs.com/img/202206132224289.png)
...
![image-20220613222930000](https://blog-images-djx.oss-cn-hangzhou.aliyuncs.com/img/202206132229065.png)
...
4、一个是以秒为单位的时间戳，一个是更精确的纳秒精度。它和`System.currentTimeMillis()`返回的`long`相比，只是多了更高精度的纳秒。

5、`Instant`就是时间戳，那么，给它附加上一个时区，就可以创建出`ZonedDateTime`：
...
[使用Java 8新增的Stream操作Collection集合 ](http://c.biancheng.net/view/6805.html) 

[JDK8 Stream ](https://blog.csdn.net/Al_assad/article/details/82356845)  
...
//只要是个排序，遍历输出
        ...
        // 串行流，过滤长度3的前三个
        Stream streamOne = strings.stream().filter( s -> (s.length() == 3) ).limit(3);

        // 过滤开头为张的，并且跳过前俩的
        Stream streamTwo = strings.stream().filter(s -> s.startsWith("张")).skip(2);

        // 合并one、two 然后
        Stream.concat(streamOne, streamTwo).map(s->new Person2((String) s)).forEach(System.out::println);
        System.out.println("--------------");
        strings.stream().map(s->new Person(s)).forEach(System.out::println);
    }

}


public class Person implements Function<String,Person> {
    String name;
    @Override
...
public class Person2 implements Function<Stream, Person2> {
    String name;
    @Override
    public Person2 apply(Stream stream) {
        return null;
    }
```
